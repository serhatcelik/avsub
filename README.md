...
```
usage: avsub [-h] [+a CODEC] [-A] [--compress VALUE] [--copy STREAM [STREAM ...]]
             [-H] [-i] [-L] [--remove STREAM [STREAM ...]] [+s CODEC] [--speed PRESET]
             [-S] [+v CODEC] [-v] [-V] [-b] [--color1 COLOR] [--color2 COLOR] [--embed SUBTITLE]
...
optional arguments:
  ...
  -S, --subtitle        choose subtitle stream only
  +v CODEC              set CODEC as output video codec
  -v, --version         show program version and exit
  -V, --video           choose video stream only
